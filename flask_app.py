    ...
    if any_from(('помо', 'help'), in_=command):
        ...
        if 'вернуться' in command or 'назад' in command or 'основ' in command or 'домой' in command or 'начало' in command:
            ...
            if 'вод' in command or 'баланс' in command:
                ...
                answer_options = [' Идеальный вес -  это не только красивая фигура, но и здоровье и работоспособность. Подскажите, пожалуйста, Ваш пол, чтобы я могла сделать точный расчёт.',
                                  'Вес влияет на продолжительность нашей жизни. Именно поэтому так важно знать его рекомендуемую норму. Не беспокойтесь, я рассчитаю Вашу индивидуальную рекомендацию. Подскажите пожалуйста Ваш пол.']
                resp.update({
                    'response': {
                        'text': f'{random.choice(answer_options)}',
                        'card': {
