...
from logging_ import logger, logged
...
def _main():
    ...
    if any_from(('помо', 'help'), in_=command):
        ...
        resp.response = ResponseField(
            text=state.help_message if state is not None else MainGroup.help_message,
            ...
            if 'вод' in command or 'баланс' in command:
                ...
                resp.response = ResponseField(
                    text=[
                        ...
                        'Так как мы состоим из воды примерно на 70% 🐳, то употреблять её в достаточном количестве очень '
                        'важно.\n'
                ...
                answer_options = [
                    ' Идеальный вес -  это не только красивая фигура, но и здоровье и работоспособность. Подскажите, пожалуйста, Ваш пол, чтобы я могла сделать точный расчёт.',
    ...
    if resp.get('response', None) is None:
        resp['response'] = {'text': f'Не совсем поняла вас. {state.help_message}'}
