    ...
    if state == MainGroup.Water.state_1:
        ...
        for el in li:
            ...
            if el.replace('.', '').isdecimal() and el.count('.') <= 1:
                answer_options = [
                    f'Ваше минимальное потребление воды {int(float(el) * 30)} миллилитров в день 💦',

                    f'Вам необходимо {int(float(el) * 30)} миллилитров воды 🌊 в день, для хорошего метаболизма. ']
                resp.update({
                    'response': {
