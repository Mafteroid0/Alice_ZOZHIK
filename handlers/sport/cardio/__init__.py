...
def _start_rope_cardio(context: FSMContext, resp: dict | Response) -> dict | Response:
    resp.update({
        'response': {
            'text': 'Давайте приступим к кардиотренировке. Для нее Вам понадобится только скакалка и хорошее '
                    'настроение. Одно упражнение занимает 40 секунд. Перед тем, как его проделать, Вы можете изучить '
                    'технику подробнее, начать выполнение или пропустить его и перейти к следующему.Вы готовы к '
                    'кардио или подберём другую тренировку?',
            'card': {
                'type': 'ItemsList',
                'header': {
