...
class MainGroup(StatesGroup):  # Состояние по умолчанию это None, его не нужно явно определять
    ...
    main_menu = State(_help_message='{now} Вы можете выбрать себе занятие из предоставленного списка: "Спортивные '
                                    'тренировки", "Водный баланс", "Фазы сна", "Идеальный вес"')
    ...
    class Water(StatesGroup):
        ...
        end = State(_help_message='{now} Вы можете ещё раз выполнить вычисление, или "Вернуться к основному списку", '
                                  'то есть в меню.')
    ...
    class Sport(StatesGroup):
        ...
        class Wrap(StatesGroup):
            ...
            class WarmDown(StatesGroup):
                ...
                qw = State(
                    _help_message='{now} Вы можете "Начать заминку". Также можно вернуться '
                                  'обратно к'
        ...
        class Power(StatesGroup):
            _help_message = '{now} Вы можете "Выполнить упражнение", узнать "Подробнее" о технике его выполнения или "Пропустить" и перейти к следующему'
        ...
        class Cardio(StatesGroup):
            _help_message = '{now} у Вас есть выбор между классической (вызывается командой "классическая") и тренировкой с ' \
            ...
            class Solo(StatesGroup):
                ...
                task1_help = State()
                task1_do = State(
                    _help_message='{now} Вы можете перейти к следующему упражнению, попросив меня об этом.')
                task2 = State()
                task2_help = State()
                task2_do = State(
                    _help_message='{now} Вы можете перейти к следующему упражнению, попросив меня об этом.')
            ...
            class Rope(StatesGroup):
                ...
                start = State(_help_message='{now} Вы можете начать выполнение, сказав "Готов" или "Вернуться в меню" и выбрать другое занятие'
                )
                task1 = State()
                task1_help = State()
                task1_do = State(
                ...
                task5_do = State(
                    _help_message='{now} Вы можете перейти к следующему упражнению, попросив меня об этом.')
                end = State(_help_message='{now} Вы можете "Выполнить заминку" или "Завершить тренировку"')
                final = State(_help_message='{now} Вы можете "Повторить тренировку" или "Завершить тренировку"')
