...
def main():
    ...
    if req['session']['new']:
        ...
        if fsm.get_state(user_id) == None and (command == 'что ты умеешь'):
        ...
        elif (fsm.get_state(user_id) == MainGroup.state_1  or fsm.get_state(user_id) == None) and command == 'поехали':  # TODO: Добавить в условия номера стейтов, из которых можно сюда попасть (см. диаграмму)
                ...
                if fsm.get_state(user_id) == MainGroup.SportBranch.Water.state_1:
                    ...
                    for el in li:
                        ...
                        if el.replace('.', '').isdecimal() and el.count('.') <= 1:
                            answer_options = [
                                f'Ваше минимальное потребление воды {float(el) * 0.03} литра в день 💦',

                                f'Вам необходимо {float(el) * 0.03} литра воды 🌊 в день, для хорошего метаболизма. ']
                            res = {
                                'version': req['version'],
                                'session': req['session'],
                                'response': {
                    ...
                    elif (fsm.get_state(user_id) == MainGroup.SportBranch.Water.end and ('ещё' in command or 'счит' in command)):
                        res = {
                            'version': req['version'],
                            'session': req['session'],
                            'response': {
