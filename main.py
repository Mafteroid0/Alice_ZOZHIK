...
def main():
    ...
    elif (fsm.get_state(user_id) == MainGroup.state_1 or fsm.get_state(user_id) is None) and ('поехали' in command or 'нач' in command):  # TODO: Добавить в условия номера стейтов, из которых можно сюда попасть (см. диаграмму)
