...
def main():
    ...
    elif (fsm.get_state(user_id) == MainGroup.state_1 or fsm.get_state(user_id) is None) and (
        ...
        if fsm.get_state(user_id) == MainGroup.SportBranch.state_home:
            if 'вод' in command or 'баланс' in command:
                answer_options = [
                    ...
                    'Так как мы состоим из воды примерно на 70%, то употреблять её в достаточном количестве очень важно. Не переживайте, я подскажу, какое минимальное количество Вам необходимо выпивать в течение дня 😉 Подскажите, пожалуйста, Ваш вес.']
                ...
                res.update({
                    'response': {
                        'text': f'Извините, не поняла вас😣\nДавайте попробуем заново выбрать занятие!\n'
                                f'"Кардиотренировка", "Силовая тренировка", "Утренняя зарядка", "Водный баланс", или "Фазы сна".',
                        'card': {
                            'type': 'ItemsList',
                            'header': {
                        ...
                        res.update({
                            'response': {
                                'text': 'Давайте приступим к кардиотренировке. Для нее вам не понадобится дополнительный инвентарь,'
                                        ...
                                        'Во время треннировки вы можете изучить упражнение подробнее, выполнить его, или пропустить выполнение и перейти к следующему. '
                                        'Вы готовы начать, или рассмотрим другую треннировку?',
                                'buttons': [
                                    {
                                        'title': 'Я готов💯',
                                        'hide': True
